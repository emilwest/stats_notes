...
```{r cdf, message=F, warning = F, cache=T}
options(scipen = 999)
...
```{r qq_plot, message=F, warning = F, cache=T}
...
The following function works for data frames in long format where there is a 'value'
column. If you supply a reference data frame, this function detects which values in 
the supplied column are missing and adds them with values as NA.

```{r complete_missing, message=F, warning = F}
library(tidyverse)
