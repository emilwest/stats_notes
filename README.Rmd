...
In this case, for p = 0.5, we get that the quantile equals the mean = median = 50, since the normal distribution is symmetric and n is large.

TODO: show qq-lot

## The difference between percentile, quantile and quartile

These are the sets of possible values (domains) for the quantile(x) and percentile(x) functions, while the quartile is always between 0-4.
...
If the csv files has different columns, read directly into a list with map() instead.
...
>>>>>>> e363db74cdca41b82f1734335c2c249ed6ae4ae7
```
...
```{r targetpace, warning=F, message=F, collapse=TRUE}
library(tidyverse)
